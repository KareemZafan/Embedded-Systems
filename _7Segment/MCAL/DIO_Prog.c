...
/* They are static to limit there scope */
/****************************************************************************************/
...
u8 DIO_voidInit(void) {
	...
	*(DIO_dirs[DIO_PORTA]) = DIO_DEFAULT_PORT_DIRECTION;
	*(DIO_dirs[DIO_PORTB]) = DIO_DEFAULT_PORT_DIRECTION;
	*(DIO_dirs[DIO_PORTC]) = DIO_DEFAULT_PORT_DIRECTION;
	*(DIO_dirs[DIO_PORTD]) = DIO_DEFAULT_PORT_DIRECTION;
...
u8 DIO_u8SetPortDirection(u8 Copy_portNumber, u8 Copy_portDirectionMode) {
	...
	if (Copy_portNumber >= DIO_PORTA && Copy_portNumber <= DIO_PORTD) {
		switch (Copy_portDirectionMode) {

		case DIO_PORT_OUTPUT:
			*(DIO_dirs[Copy_portNumber]) = DIO_PORT_OUTPUT;
			API_Status = EXIT_SUCCEED;
			break;

		case DIO_PORT_INPUT:
			*(DIO_dirs[Copy_portNumber]) = DIO_PORT_INPUT;
			API_Status = EXIT_SUCCEED;
			break;
		default:
...
u8 DIO_u8SetPintDirection(u8 Copy_portNumber, u8 Copy_pinNumber, u8 Copy_pinDirectionMode) {
	u8 API_Status = EXIT_FAILED;
	if (Copy_portNumber >= DIO_PORTA && Copy_portNumber <= DIO_PORTD) {
...
u8 DIO_u8GetPinValue(u8 Copy_portNumber, u8 Copy_pinNumber , u8 *Copy_pinValue)
{
	u8 API_Status = EXIT_FAILED ;
	if (Copy_portNumber >= DIO_PORTA && Copy_portNumber <= DIO_PORTD){
		if (Copy_pinNumber >= PIN0 && Copy_pinNumber <= PIN7) {
           *Copy_pinValue = GET_BIT((*(DIO_pins[Copy_portNumber])),Copy_pinNumber) ;
	}else {
		API_Status = EXIT_FAILED ;
	}
} else {
